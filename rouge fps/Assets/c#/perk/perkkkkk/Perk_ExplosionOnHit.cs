...
public sealed class Perk_ExplosionOnHit : MonoBehaviour
{
    ...
    // Hard guard against re-entry (extra safety).
    private bool _reentryGuard;

    // Dedupe targets so multi-collider enemies don¡¯t get hit multiple times.
    ...
    private void HandleHit(CombatEventHub.HitEvent e)
    {
        ...
        // Only react to hits from the gun that owns this perk.
        if (e.source != _sourceGun) return;

        // Ignore hits that are explicitly marked to skip hit events.
        ...
        try
        {
            for (int i = 0; i < count; i++)
            {
                ...
                // Strong-typed enemy filter + dedupe
                int targetId;
                ...
                else
                {
                    ...
                    // IDamageableEx is an interface; try to map it back to a Component for stable ID.
                ...
                // Critical fix: prevent infinite OnHit recursion.
